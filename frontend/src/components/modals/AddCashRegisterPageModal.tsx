...
export default function AddCashRegisterPageModal({ isOpen, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    try {
      if (selectedDate) {
        ...
        await dispatch(addCashRegisterPage(date));
      }
      onClose();
    } catch (error) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[500px] p-6">
      <div className="flex flex-col gap-4">
        <h3 className="text-lg font-medium text-gray-800 dark:text-white/90">
          Add New Cash Register Page
        </h3>
        
        <div className="mt-4">
          <DatePicker
            id="cash-register-page-date"
            label="Select Date"
